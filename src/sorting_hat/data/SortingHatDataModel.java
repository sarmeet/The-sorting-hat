...
public class SortingHatDataModel extends MiniGameDataModel
{
    ...
    public void playWinAnimation()
    {
        ...
        winPath.add((viewport.getScreenWidth()/2)-100);
        winPath.add((viewport.getScreenHeight()/2)-100); // mid-top left
        
        winPath.add(100);
        winPath.add((viewport.getScreenHeight()/2)); // mid-left
        
        winPath.add((viewport.getScreenWidth()/2)-300);
        winPath.add((viewport.getScreenHeight()/2)+100);// lower left
        
        winPath.add(300);
        winPath.add(viewport.getScreenHeight()-30); //Lower left cone
        
        winPath.add(viewport.getScreenWidth()/2);
        winPath.add((viewport.getScreenHeight()/2)+100);//mid-low
        
        winPath.add((viewport.getScreenWidth()/2)+300);
        winPath.add(viewport.getScreenHeight());//lower-right-cone
        
        winPath.add((viewport.getScreenWidth()/2)+100);
        winPath.add((viewport.getScreenHeight()/2)+100); // upper-lower-right
        
        winPath.add((viewport.getScreenWidth())-200);
        winPath.add((viewport.getScreenHeight())/2);
        
        winPath.add((viewport.getScreenWidth()/2)+100);
        winPath.add((viewport.getScreenHeight()/2)-100);
    ...
    public void endGameAsWin()
    {
        ...
        // RECORD IT AS A PERFECT WIN
        if (this.getBadSpellsCounter() == 0){
            
             ((SortingHatMiniGame) miniGame).getPlayerRecord().addPerfectWin(currentLevel);
             ((SortingHatMiniGame) miniGame).getPlayerRecord().addPerfectWinTime(
                     gameTime,gameTimeToText(), currentLevel);
    ...
    public void reset(MiniGame game)
    {
        ...
        for (SortingHatTile tile : stackTiles)
        {
            tile.setX(ALGORITHM_TILE_X);
            tile.setY(ALGORITHM_TILE_Y);
            tile.setState(SortingHatTileState.VISIBLE_STATE.toString());
        }
    ...
    public SortTransaction getPreviousSwapTransaction() {
       SortTransaction st = null;
        if ( transactionCounter>0){
        transactionCounter = transactionCounter -1;
             st =  properTransactionOrder.get(transactionCounter);
        }
