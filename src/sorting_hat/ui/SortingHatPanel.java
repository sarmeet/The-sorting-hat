...
public class SortingHatPanel extends JPanel {
    ...
    public void renderStats(Graphics g) {
        ...
        if (((SortingHatMiniGame) game).isCurrentScreenState(GAME_SCREEN_STATE)
            ...
            // RENDER THE TILES LEFT
            g.setFont(FONT_ALGORITHM_DISPLAY);
            g.setColor(Color.WHITE);

            // RENDER THE ALGORITHM
            String currentLevelForAlgorithm = data.getCurrentLevel();
            String currentAlgorithm = record.getAlgorithm(currentLevelForAlgorithm);
            if( currentAlgorithm.endsWith("BUBBLE_SORT")){
                currentAlgorithm = "Bubble Sort";
            }
            else currentAlgorithm = "Selection Sort";
            int x = ALGORITHM_TILE_X + ALGORITHM_DISPLAY_OFFSET_X;
            int y = ALGORITHM_TILE_Y +ALGORITHM_DISPLAY_OFFSET_Y;
            g.drawString(currentAlgorithm, x, y);
            
        }
        
        
            // DISPLAY THE MISCOUNT COUNTRE 
            int misCount_X = TILE_COUNT_X + TILE_COUNT_OFFSET;
            int misCount_Y = TILE_COUNT_Y + TILE_TEXT_OFFSET;
            String misCount = Integer.toString(data.getBadSpellsCounter());
            g.setColor(Color.BLACK);
            g.setFont(FONT_TEXT_DISPLAY);
            g.drawString(misCount, misCount_X, misCount_Y);
        ...
        if (game.getGUIDialogs().get(STATS_DIALOG_TYPE).getState().equals(SortingHatTileState.VISIBLE_STATE.toString())) {
            ...
            String perfectWinTime;
            if ( record.getPerfectWinsTime(currentLevel) == null){
                perfectWinTime = "";
            }
            else{
                perfectWinTime = record.getPerfectWinsTime(currentLevel);
            }
            ...
            g.drawString(fastestPerfectWinPrompt + perfectWinTime ,STATS_LEVEL_X, STATS_FASTEST_PERFECT_WIN_Y);
        }
    }
